...
### Второй формат источника: готовый zip-набор

Некоторые источники (например, футбольные предсказания) вместо россыпи
сообщений с манифестом присылают один **zip-файл** с уже готовым, разложенным
по площадкам набором:

```
kit.json          — что куда идёт: платформа -> список картинок + текстовый файл
instagram/        — 01-picks.png, 02-analysis.png, caption.txt
threads/          — свои картинки и текст (если площадка используется)
x/                — свои картинки и post.txt
```

Это принципиально другой путь, и он идёт **в обход** пачек/манифеста/выбора
варианта (`selector.py`): раз в архиве уже сказано, какая картинка и текст
относятся к какой площадке, гадать (`SELECT_STRATEGY`, `longest`/`shortest`)
незачем. Как только такой zip падает в чат:

1. `telegram_source._is_zip()` узнаёт его по MIME (`application/zip`) или
   расширению `.zip` — раньше, чем сообщение попадёт в старую манифест-логику.
2. Архив скачивается и распаковывается на Volume, читается `kit.json`.
3. Для каждой площадки из `kit.json` (`threads`/`instagram`/`x`) картинки
   регистрируются в медиа-реестре, текст читается из указанного файла.
4. Тут же создаётся пачка со статусом `pending` (`db.start_zip_burst`) —
   **без** ожидания `BURST_WAIT_SECONDS`: содержимое уже полное, ждать
   нечего. Ближайший тик воркера (`WORKER_INTERVAL_SECONDS`, по умолчанию
   10 сек.) публикует её.
5. Публикует `worker._process_zip_burst()` — использует те же функции
   финализации (`_finalize_threads/_instagram/_x`: хештеги, лимит символов,
   разбивка на тред у X), что и обычный путь, только без вызова
   `selector.choose()`.

Если в архиве нет варианта для включённой площадки (например, прислали
только `threads`, а `X_ENABLED=true`) — эта площадка у конкретного поста
просто пропускается, без ошибки. Ретраи и `/admin/requeue` работают
одинаково для обоих форматов источника.

Добавить такой источник — то же самое, что раньше делали для эспорта и UFC:
дописать id чата через запятую в `SOURCE_CHAT_ID`. Тип чата (канал или
группа) роли не играет — Telethon подписывается одинаково; никакой отдельной
настройки для "zip-режима" не требуется, формат определяется по каждому
сообщению отдельно, так что канал может параллельно слать и старый формат,
и новый.
